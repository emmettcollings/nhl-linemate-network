...
def format_lines_df(df):
    ...
    df = df.drop(columns=["season", "iceTimeRank", "team", "situation"])
...
def add_line_data(line_row):
    ...
    if line_row.name in lines_df.index:
        ...
        ) + updated_row.corsiPercentage * (updated_row.icetime / total_icetime)
...
split_ids = (
    lines_df.index.to_series().apply(split_string).fillna("")
)  # Returns pd.Series
split_ids.columns = "playerId" + (split_ids.columns + 1).astype(str).str.zfill(1)
...
# Write to file
output_file = os.path.join(dirname, "../../../data/interim/aggregated_lines.csv")
lines_df.to_csv(output_file)
output_file = os.path.join(dirname, "../../../data/final/aggregated_lines.csv")
lines_df.to_csv(output_file)
